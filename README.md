...
### Target Directory (where generated codes are placed)
...
* The code-template-tool will infer identifier style based on each variable, and convert the replacer to proper style.
...
* You can also set variables' style in config file.

  * variable identifier in config file: `abcDef`, `ijkLmn`, `opqRst`, `uvwXyz`
  ...
  * variable value set by user: 

    | variable name | value set by user |  
    | ------------- | ----------------- |  
    | `abcDef`      | `userInfo`        |  
    | `ijlLmn`      | `userProfile`     |  
    | `opqRst`      | `UserName`        |  
  ...
  * variable in templates and result in generated codes
    ...
    | variable in templates | result in generated codes |  
    | --------------------- | ------------------------- |  
    | `___abcDef___`        | `___useInfo`              |  
    | `___ijkLmn___`        | `UserProfile___`          |  
    | `___opqRst___`        | `__user_name__`           |  
    | `___uvwXyz___`        | `user-gender`             |  
...
* The config object's fields:  
  ...
  * `variables`: An array of a template's variable names or configs. 

    ```json
    "variables": [
        "name": "variableName",
        "case": "variableCaseStyle",
        "prefixUnderscore": 2, // number of prefix underscores
        "suffixUnderscore": 3  // number of suffix underscores
    ]
    ```

    If not configed, `prefixUnderscore` and `suffixUnderscore` will be 0 by default, `case` can be inferred based on variables in template content.
